...
function change_pom_no_listener() {
    ...
    if [[ ! ${OPTION} == *"NoDSI"* ]]; then
        ...
        ${MOP_CHANGER} `pwd` ${SCRIPT_DIR}/ruleverify.jar
    else
...
function run_optimized_dsi_plus() {
    ...
    cat ${master_spec_file} | sed "s/$/ ${OPTIMIZATION_LEVEL}/g" | sed 's/ /@/g' | parallel -j ${PARALLEL_COUNT} bash ${SCRIPT_DIR}/run-parallel-optimized-dsi-plus.sh
